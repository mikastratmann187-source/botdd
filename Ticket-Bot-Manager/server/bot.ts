...
dotenv.config(); // Lädt lokale .env für lokale Tests, Render liest automatisch Environment Variables
...
if (!TOKEN) {
  console.error('DISCORD_TOKEN ist nicht gesetzt! Bitte in Render oder .env eintragen.');
  process.exit(1);
}
...
// ----------------------
// HELPER: SICHER INTERACTION AUFRUFEN
// ----------------------
  ...
  try {
    if (GUILD_ID && client.application?.owner) {
      ...
      if (guild) {
        ...
        console.log(`✅ Commands registered to guild ${GUILD_ID} via REST`);
      }
    } else {
  ...
  try {
    ...
    if (typeof interaction.isButton === 'function' && interaction.isButton()) {
      if (interaction.customId === 'ticket_close') {
        ...
        return; // nach showModal immer return
      }
    }
